...
void SubjectGlucose::EvolutionUnderFoodIntakeExerciseEvents(){
    ...
    if (foodIntakeExerciseSaILaIEvents.size() < 2) {
        std::cout << "ERROR(SubjectGlucose::EvolutionUnderFoodIntakeExerciseEvents): the size of 'foodIntakeExerciseSaILaIEvents' must be larger than 1" << std::endl;
        return;
    }
...
//        eDES_ex_med.SetEvolutionSpecifics(std::make_tuple(type, bodyMass, fastGlucose, fastInsulin, this_meal,
...
//        // {t_curr, MGgut_curr, Gpl_curr, Ipl_curr, Int_curr, ExPre_curr, Ex_curr, UI_sc1_curr, UI_sc2_curr}
//        if (evol == 0) initialConditions_tmp = {tI_tmp, 0., fastGlucose, fastInsulin, fastGlucose-Gpl_basal, 0., ex_intensity, 0., 0.};
//        else{
//            initialConditions_tmp = eDES_ex_med.GetCurrentEvolvedParams();
//            initialConditions_tmp[5] = initialConditions_tmp[6]; // in the new run, ExPre_init = (Ex from the previous run)
//            initialConditions_tmp[6] += ex_intensity; // add new ex_intensity
//        }
//        eDES_ex_med.SetInitConditions(initialConditions_tmp);
//        eDES_ex_med.SetCheckPts(tI_tmp, tF_tmp, evol_steps);
//        // perform evolution
