...
public:
    ...
    // set the vars. specific to the evolution: <type, bodyMass, Dmeal, exercise, Gpl_basal, Ipl_basal>
    void SetEvolutionSpecifics(const std::tuple<int, double, double, double, double, double> &specifics);
    ...
    std::vector<double> GetCurrentEvolvedParams() {return {t_curr, MGgut_curr, Gpl_curr, Ipl_curr, Jpl_curr, Iif_curr}; }
...
private:
    ...
    // Evolution specifics:
    //      type -- type of the subject, which determines the set of 'fitted-params'
    //      Dmeal -- amount of food intake
    //      Mb -- body mass
    int type = 0;
    double Mb = 75.; // kg
    double Dmeal = 75E3; // mg
    double exercise = 0.;
