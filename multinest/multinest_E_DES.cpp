...
void E_DES::SetSubjectTypeFittedParams(const int &type){
    switch (type) {
        case 0: // healthy person
            k1 = 1.45E-2;
            k2 = 2.76E-1;
            k3 = 6.07E-3; // tweaked params
            k4 = 2.35E-4;
            k5 = 9.49E-2; // tweaked params
            k6 = 1.93E-1; // tweaked params
            k7 = 1.15; // tweaked params
            k8 = 7.27;
            k9 = 0.;         // short-acting insulin
            k10 = 0.;        // short-acting insulin
            k11 = 3.83E-2; // tweaked params
            k12 = 2.84E-1;
            sigma = 1.34;
            KM = 13.0995;
            break;
...
//    std::cout << "SSR_1 = " << SSR << std::endl;
//    // add constraints for the min_parameters: all parameters > 0; add penalties if any of them is negative
//    for (int i = 0; i < minParams_fval_curr.size() - 1; ++i) {
//        SSR += pow(minParams_fval_init[i] / (minParams_fval_curr[i] + 0.001 * minParams_fval_init[i]), 1.);
//    }
//    std::cout << "SSR_2 = " << SSR << std::endl;
...
//    std::vector<double &> min_paramsRef = {k1, k2, k3, k4, k5, k6, k7, k8, k11, k12, sigma, KM};
    for (int i = 0; i < min_paramsP.size(); ++i) {
        *min_paramsP[i] = pow( 10, SS_tmp[i] + SD_tmp[i] * tanh( gsl_vector_get(v, i)) );
//        min_paramsRef[i] = pow( 10, SS_tmp[i] + SD_tmp[i] * tanh( gsl_vector_get(v, i)) );
    }
...
void E_DES::EstimateFittedParameters_gsl(){
    ...
    //  1. parameter transformation: in order to satisfying the constraints that all params > 0
    ...
    log10_min_params_range[0] = { 0.5*log10(k1), 2.*log10(k1) }; // k1
    log10_min_params_range[1] = { 0.5*log10(k2), 2.*log10(k2) }; // k2
    log10_min_params_range[8] = {-1.E99, -1.E99}; // k9
    log10_min_params_range[9] = {-1.E99, -1.E99}; // k10
    ...
    for (int i = 0; i < log10_min_params_range.size(); ++i) {
        double x = log10(minParams_init[i]);
    ...
    //compare with the initial parameter
    std::cout << "init ";
    for (int i = 0; i < minParams_init.size(); ++i) {
        std::cout << minParams_init[i] << " ";
    }
    std::cout << std::endl;
